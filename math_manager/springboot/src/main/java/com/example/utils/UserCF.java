...
public class UserCF {
	...
	public static List<Integer> recommend(Integer userId, List<RelateDTO> list) {
		...
		// 最近邻用户看过课程列表
		List<Integer> neighborItems = userMap.get(nearestUserId).stream().map(RelateDTO::getCourseId).collect(Collectors.toList());
		// 指定用户看过课程列表
		List<Integer> userItems = userMap.get(userId).stream().map(RelateDTO::getCourseId).collect(Collectors.toList());
		// 找到最近邻看过，但是该用户没看过的商品
